...
class ZoneServices:
    ...
    async def stop_zone(self, zone_id: int) -> tuple:
        """Stop watering a specific zone.

        Returns:
            tuple: (success: bool, schedule_queue: list, schedule_id: str|None),
                mirroring ``SprinklerSystem.stop_zone``.
        """
    ...
    async def update_zone_settings(self, zone_id: int, **settings: Any) -> bool:
