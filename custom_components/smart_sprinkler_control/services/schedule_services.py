...
class ScheduleServices:
    ...
    async def update_schedule(self, schedule_id: str, **updates: Any) -> bool:
        ...
        for zone_id in schedule.zone_ids:
            if zone_id in self.system.zones:
                ...
                if not self.system.start_zone(zone_id, duration, schedule_id):
                    success = False
                    _LOGGER.warning(
                        "Failed to start zone %d for schedule %s", zone_id, schedule_id
                    )
    ...
    async def get_schedule_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "enabled_schedules": len(
                [s for s in self.system.schedules.values() if s.enabled]
            ),
            "schedules": {
                schedule_id: {
                    "name": schedule.name,
